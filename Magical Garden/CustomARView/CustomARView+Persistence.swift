...
extension CustomARView {
    ...
    func loadExperience() {
        ...
        virtualObjectAnchors.removeAll()
    }
    
    func saveExperience() {
        self.session.getCurrentWorldMap { worldMap, error in
            guard let map = worldMap else {
                self.arState.sessionInfoLabel = "Can't get current world map: \(error?.localizedDescription ?? "unknown error")"
                return
            }
